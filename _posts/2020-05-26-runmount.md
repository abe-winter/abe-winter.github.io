...
Ran a quick test of something similar, success (uncached would take 9.5s to download modules and ~30s to build):
...
=> [stage-1  8/11] RUN --mount=type=cache,id=gobuild,target=/root/.cache/go-build --mount=type=cache,id=gopkg,target=/go/pkg go build ./cmd/...
```
