...
Lean Startup (rest easy, that's not an affiliate link) is one of those books that if it comes up in a conversation,
...
product-first companies should be managed as products, not as 'traditional businesses' (my phrase not his).
And therefore you shouldn't do financial planning until your product has gone through a release cycle.
...
If the latter, it could be an index card; and certainly it seems like the author was seriously influenced by Steve Blank's (see 'books it draws from' section).
...
Another good point here is that 'viral, stick, and paid growth models' are different,
...
'Knowledge work is actually just factory work' is a daring metaphor and ultimately a weak model
because factory work is about optimizing repeatable processes with known resource constraints.
(I learned from the book that kanban means 'capacity constraint', which is delightful and also unfortunately not substantiated by a dictionary).
...
Especially not for the form of high-uncertainty product development Ries seems to be focusing on.
